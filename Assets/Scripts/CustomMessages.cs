...
namespace HoloToolkit.Sharing.Tests
{
    ...
    public class CustomMessages : Singleton<CustomMessages>
    {
        ...
        private Dictionary<CustomMessageID, MessageCallback> messageHandlers = new Dictionary<CustomMessageID, MessageCallback>();
        ...
        public void BroadcastMessage(CustomMessageID messageId)
        {
            //StartCoroutine(broadcastMessages(messageId));
        ...
        public void SendTargetTransform(Vector3 position, Quaternion rotation)
        {
            // If we are connected to a session, broadcast our head info
            if (serverConnection != null && serverConnection.IsConnected())
            {
                // Create an outgoing network message to contain all the info we want to send
                NetworkOutMessage msg = CreateMessage((byte)CustomMessageID.TargetDragged);
